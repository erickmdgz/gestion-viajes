...
| TC-004 | FEAT-002 | FR-002 | Board marks a confirmed participant's F2 as complete | `F2_complete = true`, `F2_completed_at` set, `current_state = "F2 complete"` | Passed (automated: `src/lib/participants.test.ts`; manual: curl-verified via the roster page against the running app) |
| TC-005 | FEAT-002 | FR-002 | Inspect participant record after F2 completion | No F2 sensitive field (passport, birth date, medical, diet) is persisted in Solanum | Passed (automated: `participants.test.ts` structurally asserts no such keys exist on the record — guaranteed by the schema never defining those columns) |
...
| TC-034 | FEAT-010 | FR-016 | Board requests promotional content | Draft content ideas and image prompts generated | Passed (automated: `src/lib/promo.test.ts`; manual: verified 5 distinct ideas render at `/dashboard/trips/{tripId}/promo` against the running app, trip name and deadline correctly interpolated) |
